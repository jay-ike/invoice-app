               ...
               <form ref={components.form} id="invoice_form" action="" class="grow-2 y-scrollable" onInput={handleInput} on:dateselected={handleDateSelection}>
                 <step-by-step out-indicator="step-out" on:indexupdated={stepUpdater}>
                   <fieldset class="input-step stack" name="senderInfos">
                        ...
                        <div><input type="text" id="bf-street" name="senderAddress" value={dataSignal().senderAddress ?? ""} autocomplete="address-line1"/><label for="bf-street">street address</label></div>
                        <div><input type="text" id="bf-city" name="senderCity" value={dataSignal().senderCity ?? ""} autocomplete="address-level2"/><label for="bf-city">city</label></div>
                        <div><input type="text" id="bf-post-code" name="senderPostCode" value={dataSignal().senderPostCode ?? ""} autocomplete="postal-code"/><label for="bf-post-code">post code</label></div>
                        <div><input type="text" id="bf-country" name="senderCountry" value={dataSignal().senderCountry ?? ""} autocomplete="country-name"/><label for="bf-country">country</label></div>
                    </fieldset>
                    <fieldset class="input-step stack" name="clientInfos">
                        <legend>bill to</legend>
                        <div><input type="text" id="bt-name" name="clientName" value={dataSignal().clientName ?? ""} autocomplete="name" data-new required/><label for="bt-name" data-error="can't be empty">client's name</label></div>
                        <div>
							<input type="email" id="bt-email" name="clientMail" value={dataSignal().clientMail ?? ""} autocomplete="email" data-new required />
							<label for="bt-email" data-error="invalid email address">client's email</label>
						</div>
                        <div><input type="text" id="bt-street" name="clientAddress" value={dataSignal().clientAddress ?? ""} autocomplete="address-line1"/><label for="bt-street">street address</label></div>
                        <div><input type="text" id="bt-city" name="clientCity" value={dataSignal().clientCity ?? ""} autocomplete="address-level2"/><label for="bt-city">city</label></div>
                        <div><input type="text" id="bt-post-code" name="clientPostCode" value={dataSignal().clientPostCode ?? ""} autocomplete="postal-code"/><label for="bt-post-code">post code</label></div>
                        <div><input type="text" id="bt-country" name="clientCountry" value={dataSignal().clientCountry ?? ""} autocomplete="country-name"/><label for="bt-country">country</label></div>
                    </fieldset>
                    <fieldset class="input-step stack" name="terms">
                        <legend>payment terms</legend>
                        <date-picker month-format="short" locale="en-GB">
                            <input type="text" id="inv-date" name="invoiceDate" value={dataSignal().invoiceDate ?? ""} autocomplete="off" data-new required/>
                            <label for="inv-date" data-error="no date selected">invoice date</label>
                        </date-picker>
                        <div>
                            <select name="paymentTerms" value={dataSignal().paymentTerms ?? ""} id="inv-terms" autocomplete="off" data-new required>
                        ...
                        <div><input type="text" id="inv-desc" name="description" value={dataSignal().description ?? ""} data-new required/><label for="inv-desc" data-error="no description specified">project description</label></div>
                    </fieldset>
                    <div class="stack">
